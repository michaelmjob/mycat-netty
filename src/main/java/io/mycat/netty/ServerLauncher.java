...
import io.mycat.netty.mysql.MySQLServer;
...
//        launch(args);
    ...
    // simple  bootstrap
    public static void SimpleBoottrap(){
        ServerArgs args = new ServerArgs();
        args.port(8090);
        NettyServer server = new MySQLServer(args);
        server.init();
        server.listen();
        try {
            server.waitForClose();
        } catch (InterruptedException e) {
            logger.error("server waitforclose occur exception", e);
            System.exit(-1);
        }
    }
