package org.mycat.netty.mysql.parser;

import org.mycat.netty.util.ParseUtil;
...
public final class ServerParseSet {
    ...
    private static int sqlMode(String stmt, int offset) {
        if (stmt.length() > offset + 8) {
            char c1 = stmt.charAt(++offset);
            char c2 = stmt.charAt(++offset);
            char c3 = stmt.charAt(++offset);
    ...
    private static int policy(String stmt, int offset) {
        if (stmt.length() > offset + 6) {
            char c1 = stmt.charAt(++offset);
            char c2 = stmt.charAt(++offset);
            char c3 = stmt.charAt(++offset);
